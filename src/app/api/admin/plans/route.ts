...
const planSchema = z.object({
  ...
  familyCode: z.string().trim().min(2).max(30).regex(/^[a-z0-9-]+$/).optional(),
  interval: z.enum(["MONTHLY", "YEARLY"]).optional(),
  priceInr: z.coerce.number().int().min(0),
  productLimit: z.coerce.number().int().min(-1),
  aiEnhanceEnabled: z.boolean().optional(),
  aiCreditsPerMonth: z.coerce.number().int().min(0).max(10000).optional(),
